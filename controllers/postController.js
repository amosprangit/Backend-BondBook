...
import { sendPushNotification } from "../utils/senderNotification.js";
// Create a new post
export const createPost = async (req, res) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ message: "Please upload an image or video file." });
    }
  ...
  try {
    ...
    if (userCheck) {
      ...
      const user = await User.findById(userId).select(
        "username email profilePicture bio followers following postsCount isVerified createdAt",
      );
...
export const searchPosts = async (req, res) => {
  try {
    ...
    const scopes = (
      scopeParam ? scopeParam.split(",") : ["caption", "username", "tag"]
    ).map((s) => s.trim());
    ...
    if (hasExplicitFields) {
      ...
      if (tagname) {
        orFilters.push({
          tags: { $elemMatch: { $regex: new RegExp(tagname, "i") } },
        });
      }
      if (tagsRaw) {
        const tagsList = Array.isArray(tagsRaw) ? tagsRaw : [tagsRaw];
        const tagRegexes = tagsList
          .filter(Boolean)
    ...
    if (hasExplicitFields && usernameField) {
      const users = await User.find(
        { username: { $regex: new RegExp(usernameField, "i") } },
        { _id: 1 },
      );
      ...
      !hasExplicitFields &&
      (scopes.includes("username") || scopes.includes("user")) &&
      regex
    ) {
      const users = await User.find(
    ...
    if (orFilters.length === 0) {
      return res
        .status(200)
        .json({ message: "No results", posts: [], count: 0 });
    }
  ...
  try {
    ...
    if (
      ...
      res.status(400).json({
        success: false,
        message:
          'Invalid action. Send {"action": 1} or {"like": 1} to like. Send {"action": 0} or {"like": 0} to unlike',
      });
    }
  } catch (error) {
