...
export const loginUser = async (req, res) => {  
  try {
    ...
    // 2. CRITICAL FIX: Check for DEMO credentials BEFORE querying the Database
    if (normalizedEmail === "demo@review.com" && password === "Demo@123") {
      ...
      // Set HTTP-only cookie for demo user as well
      ...
      return res.status(200).json({
        success: true,
        message: "Demo login successful (Google Review)",
        user: {
          ...
          followingCount: 0,
          postsCount: 0,
          isVerified: true // Forces verified state
        },
        token: token
      });
    }
    ...
    try {
      ...
      const text = `Hello ${user.username}, You have successfully logged into your account at ${currentTime}.`;
      const html = `...`; // Keep your existing HTML template here
...
//             <p style="margin: 5px 0;"><strong>Session Duration:</strong> 90 days</p>
//           </div>
...
// Toggle follow - Direct follow/unfollow (no requests)
// This is for viewing stories and posts only. Does NOT create mutual connections.
// For mutual connections and messaging, use the merge request system.
...
// Send merge request (to create mutual connection)
// This creates a mutual connection when accepted, which enables:
// - Mutual connection profile
// - Messaging between users
// - Shared posts and activities
export const sendMergeRequest = async (req, res) => {
  try {
    const userId = req.user.userId;
...
// Accept merge request (creates mutual connection)
// When accepted, creates a mutual connection that enables messaging
// Users can now send messages to each other through the mutual connection
export const acceptMergeRequest = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { requestId } = req.params;

    const mergeRequest = await MergeRequest.findById(requestId);
    if (!mergeRequest) {
      return res.status(404).json({
        success: false,
...
export const checkMergeRequestStatus = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { targetUserId } = req.params;
