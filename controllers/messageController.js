...
export const sendMessage = async (req, res) => {
  try {
    const userId = req.user.userId; // ✅ This works with your userAuth
    ...
    const mutualConnection =
      await MutualConnection.findById(mutualConnectionId);
...
// Get total unread message count across all connections (for badge)
...
// Get unread messages for all connections (for chat list)
export const getUnreadMessagesForAllConnections = async (req, res) => {
  try {
    const userId = req.user.userId;

    const connections = await MutualConnection.find({
    ...
    const unreadData = await Message.aggregate([
      {
        $match: {
          mutualConnection: { $in: connectionIds },
          receiver: userId,
          isRead: false,
        },
      },
    ...
    console.error("Get unread messages for all connections error:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error",
      error: process.env.NODE_ENV === "development" ? error.message : undefined,
